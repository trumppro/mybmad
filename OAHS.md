...
```bash
# 1. Serve the spine (durable PGlite in .oahs/data). It PRINTS a generated admin
#    token at startup unless you set OAHS_ADMIN_TOKEN yourself. Postgres is NOT
#    wired to serve — the engines are `memory` and `pglite` (see docker-compose.yaml).
node apps/oahs/bin/oahs.mjs serve --data .oahs/data

# 2. Bootstrap actors (admin token) — grants are explicit; no actor holds a gate it wasn't granted
export OAHS_TOKEN=<the admin token serve printed>
