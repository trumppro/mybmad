...
> **§8–§12 (added 2026-07):** the sections below come from an architecture review against the Actorium reference (`docs/ref/ai_workflow_and_architecture.pdf`) — a 4-layer AI-infrastructure design (orchestrator / coding runtime / knowledge / agent) deployed as three independent stacks. The review confirmed the spine already exceeds the reference at the orchestrator layer (fencing tokens, evidence-gated done, entitlements as data — all things to preserve, not revisit) and found four real gaps: no knowledge layer anywhere in the plan-of-record, no credential-isolated execution runtime, a single-container topology, and an unclosed feature-layer handoff. It also found defects that outrank all of them, collected in §8. Each section respects the closed decisions above; where a section adopts an Actorium idea it says which one, and where oahs deliberately diverges it says why.
...
- **Forge integration (GitHub PR).** The ops manual's review step is "PR passes review → you look over → you merge → done"; oahs today stops at a pushed `claim/<claimId>` branch, and the merge lives outside the tool. Close it the measured-evidence way (§2.3): the runner opens a real PR from the claim branch when the item advances to `in_review` (`pr.opened` evidence), and the measuring side verifies `pr.merged_into_default` through the forge API — machine-collected facts, the core judges. Whether the done gate *requires* merged is **gate-policy data** (§3), never a hardcode; D7 unchanged (the pinned commands stay the only command guards). Until this lands, the interim runbook (merge by hand, then approve) is written down in `docs/oahs/04`.
- **Feature board & workspace UI.** The cockpit gains the ops manual's two surfaces: a features board grouped by stage (the FSM above, including `cancelled`), and a per-feature workspace with tabs — Spec / Design / Tasks / Handoff / Activity (Activity is the existing event stream). Blocked tasks show a **suggested next step** derived from the `blocked_reason` taxonomy (the runbook table in `docs/oahs/04`, surfaced in place).
...
| **11** | Knowledge layer (§11, per D12/D14): DATABASE_URL activation, SQL/pgvector memory search, spec read surface, knowledge indexer + `search_knowledge`, code-graph `impact_report` evidence, memory distillation | An agent asks "what does this feature mean / what does this change touch" over MCP and gets a cited answer; memory recall is relevance-ranked, not newest-20 | XL |
| **12** | Service topology (§12): `@oahs/auth` extraction, compose profiles as three stacks, transactional outbox, durable runner registry, webhook egress | The three compose profiles deploy and scale independently; a second service consumes `@oahs/auth` without copying code | L |
